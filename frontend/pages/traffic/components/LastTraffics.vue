...
import { formatDateBR, getWeekNumber } from "~/scripts/utils/date"
...
async function fetchData() {
  loading.value = true
  try {
    const response = await API.get(`/traffics?page=1&per_page=${limit}`)
...
<template>
  <div
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Últimas {{ limit }} semanas cadastradas</p>
    ...
    <div v-else-if="!items.length" class="text-sm text-gray-500 dark:text-gray-400">
      Nenhum tráfego cadastrado ainda.
    </div>
      ...
      <div v-for="(item, idx) in items" :key="idx"
        ...
        <div>
          <p class="font-medium dark:text-white">
            {{ getWeekNumber(item.start_date, item.end_date) }}
          </p>
        ...
        <div class="text-right mb-0">
          <p class="font-semibold dark:text-white">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-500"> Impressões
              {{ item.impressions }} -
            </p>
            <p class="text-sm text-gray-600 dark:text-gray-500 ms-1"> Cliques
              {{ item.clicks }}
            </p>
          </div>
          <p class="text-sm text-gray-600 dark:text-gray-400 font-semibold"> Investimento:
            {{ formatBRL(item.ad_investment) }}
          </p>
        </div>
      </div>
    </div>
